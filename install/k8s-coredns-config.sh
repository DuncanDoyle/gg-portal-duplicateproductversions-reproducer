...
grep -v "rewrite name portal.example.com" coredns-cm.yaml > tmpfile && mv tmpfile coredns-cm.yaml

# Add rewrites for keycloak.example.com and developer.example.com
sed <<EOF -i'.orig' -e '/ready/ i\
        rewrite name keycloak.example.com gloo-proxy-gw.ingress-gw.svc.cluster.local\
        rewrite name developer.example.com gloo-proxy-gw.ingress-gw.svc.cluster.local\
        rewrite name portal.example.com gloo-proxy-gw.ingress-gw.svc.cluster.local\
' coredns-cm.yaml
EOF
